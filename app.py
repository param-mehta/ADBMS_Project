...
def index():
    ...
    transactions = db.execute(
        "SELECT * FROM purchases WHERE id = :id AND shares<> 0 ", id=session["user_id"])
...
def buy():
    ...
    if request.method == "GET":
        ...
        if len(stocks) == 0:
            ...
            s = int(stocks[0]['shares']) + int(shares)

            db.execute(
                f"UPDATE purchases SET shares = {s}, price = {stock['price']} WHERE id = :id and stock = :stock", id=session["user_id"], stock=symbol)
...
def quote():
    ...
    if request.method == "GET":
        ...
        if not stock:
            ...
            date_,price = lookup1(name)
            date_ = [d.date() for d in date_]

        if stock is None:
            return apology("There is no stock with that name")

        return render_template("quoted.html", stock=stock, price= price, date_=date_, legend = 'price')
...
def add_cash():
    if request.method == "GET":
        ...
        if float(cash) < 1:
            return apology("Must be non-negative integer")

        if cash == "":
            return apology("You need to enter an Amount")

        new_cash = user_cash + cash

        db.execute(f"UPDATE users SET cash = {new_cash} WHERE id = :id",
                   id=session["user_id"])
    return redirect("/")
